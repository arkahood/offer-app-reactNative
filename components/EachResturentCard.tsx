...
const EachResturentCard = ({name , address, coupon_code, discount , url}:EachResturentProps) : JSX.Element => {
    ...
    const storeOffer = async() => {
        try {
            ...
            if(prevData === null) {
                await EncryptedStorage.setItem("offer",JSON.stringify({
                ...
                await EncryptedStorage.setItem("offer",JSON.stringify({
                    data : [...prevDatainJson.data, {name : name, coupon_code : coupon_code}]
                }))
            }
        } catch (error) {
    ...
    const handleRevealPromoCode = async()=>{
        ...
        if(data === null) dispatch(addOffer({data:[{name : name, coupon_code : coupon_code}]}))
        else{
    ...
    return(
        <Card>
            ...
            <TouchableOpacity style={ disabled ? styles.disablebutton : styles.button} disabled={disabled} onPress={handleRevealPromoCode}>
                <Text style={{fontSize:20, color:'white'}}>{ disabled ? 'Offer Already Redeemed' : 'Reveal Promo Code'}</Text>
            </TouchableOpacity>
        </Card>
    )
}
